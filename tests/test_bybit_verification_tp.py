...
def test_tp_lines_verified_marks_bybit_verifierad():
    ...
    assert "TP1: 103.0 (+3.00%) (Bybit verifierad)" in out
    assert "TP2: 105.0 (+5.00%) (Bybit verifierad)" in out
...
def test_tp_lines_unverified_marks_ej_pa_bybit():
    ...
    assert "TP1: 103.0 (+3.00%) (EJ på Bybit)" in out
...
def test_tp_lines_no_verified_dict_falls_back_to_no_marker():
    ...
    assert "EJ på" not in out


def test_tp_lines_blocked_marker_wins_over_verification():
    """A TP below 2% should keep the 'Blocked <2%' marker even if it
    ...
    assert "TP1: 101.0 (+1.00%) — Blocked <2%" in out
    # Should NOT also carry "(EJ på Bybit)" — that line's already
    # blocked-marked and won't have a Bybit order at all.
    assert "(EJ på Bybit)" not in out
    assert "TP2: 105.0 (+5.00%) (Bybit verifierad)" in out
...
def test_tp_lines_mixed_verified_and_no_entry_in_dict():
    ...
    lines = out.split("\n")
    assert "(Bybit verifierad)" in lines[0]
    # TP2 should have no Bybit marker (merged-into-trailing).
    assert "Bybit" not in lines[1]
...
def test_tp_lines_short_direction_works_with_verification():
    ...
    assert "TP1: 97.0 (+3.00%) (Bybit verifierad)" in out
    assert "TP2: 95.0 (+5.00%) (EJ på Bybit)" in out
