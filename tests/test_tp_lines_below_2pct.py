...
below 2% must be visible in the template with "— Blocked <2%" so the
operator sees every TP the source channel sent (proof chain) while
also seeing which ones the bot will not act on (mirrors the 2% min
distance filter in position_manager._place_partial_tp_orders).
"""
...
def test_sub_2pct_tp_is_marked_blocked_long():
    ...
    assert "Blocked" not in out.split("\n")[1]
...
def test_sub_2pct_tp_is_marked_blocked_short():
    ...
    assert "TP1: 99.0 (+1.00%) — Blocked <2%" in out
...
def test_mixed_sub_and_above_2pct():
    ...
    lines = out.split("\n")
    assert "Blocked <2%" in lines[0]
    assert "Blocked <2%" in lines[1]
    assert "Blocked <2%" in lines[2]
